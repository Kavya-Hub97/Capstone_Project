...
public class RatingsService {
	...
	public void submitRatings(Rating rating) throws InvalidInputException {
		if(ratingsRepository.findByDoctorId(rating.getDoctorId())==null){
			throw new InvalidInputException(Arrays.asList("Already Bulu"));
		}

	rating.setId(UUID.randomUUID().toString());
	ratingsRepository.save(rating);
	Double avgRating = ratingsRepository.findByDoctorId(rating.getDoctorId()).stream().collect(Collectors.averagingInt(Rating::getRating));
